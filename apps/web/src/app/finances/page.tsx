...
import { TrendingUp, TrendingDown, DollarSign, Plus, Printer, Trash2, ArrowUpCircle, ArrowDownCircle } from 'lucide-react';
...
export default function FinancesPage() {
  ...
  const deleteExpense = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['finances-summary'] }),
  });
    ...
    <ProtectedRoute allowedRoles={[UserRole.GYM_ADMIN]}>
      <DashboardLayout>
        <div className="py-6 space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div><h1 className="text-2xl font-bold text-dark">Finanzas</h1><p className="text-sm text-gray-500">Administración de ingresos y egresos</p></div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Button onClick={() => setShowPayment(true)} size="sm" className="flex items-center gap-1"><Plus className="h-4 w-4" />Ingreso</Button>
              <Button onClick={() => setShowExpense(true)} size="sm" variant="outline" className="flex items-center gap-1"><Plus className="h-4 w-4" />Egreso</Button>
            </div>
          </div>
            ...
            <Card className={`border-none shadow-sm bg-gradient-to-br ${(summary?.netProfit ?? 0) >= 0 ? 'from-primary/10 to-white' : 'from-red-50 to-white'}`}>
              <CardContent className="p-5 flex items-center gap-4">
                <div className={`p-3 rounded-xl ${(summary?.netProfit ?? 0) >= 0 ? 'bg-primary/20' : 'bg-red-100'}`}>
                  {(summary?.netProfit ?? 0) >= 0 ? <TrendingUp className="h-6 w-6 text-dark" /> : <TrendingDown className="h-6 w-6 text-red-500" />}
                </div>
                <div><p className="text-sm text-gray-500">Utilidad Neta</p><p className={`text-2xl font-bold ${(summary?.netProfit ?? 0) >= 0 ? 'text-dark' : 'text-red-600'}`}>{fmt(summary?.netProfit ?? 0)}</p></div>
              </CardContent>
            </Card>
          </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-bone">
                    <tr>{['Fecha', 'Cliente', 'Método', 'Monto', 'Acciones'].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{h}</th>)}</tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {isLoading ? <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400">Cargando...</td></tr>
                    : (summary?.payments ?? []).length === 0 ? <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400">Sin ingresos este mes</td></tr>
                    : (summary?.payments ?? []).map(p => (
                      <tr key={p.id} className="hover:bg-bone/50">
                        ...
                        <td className="px-4 py-3 text-sm font-medium">{p.membership?.client?.firstName} {p.membership?.client?.lastName}</td>
                        ...
                        <td className="px-4 py-3">
                          <button onClick={() => setPrintPayment(p)} className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-600"><Printer className="h-4 w-4" /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {(tab === 'overview' || tab === 'expenses') && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-bone">
                    <tr>{['Fecha', 'Descripción', 'Categoría', 'Monto', 'Acciones'].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{h}</th>)}</tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {isLoading ? <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400">Cargando...</td></tr>
                    : (summary?.expenses ?? []).length === 0 ? <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400">Sin egresos este mes</td></tr>
                    : (summary?.expenses ?? []).map(e => (
                      <tr key={e.id} className="hover:bg-bone/50">
                        ...
                        <td className="px-4 py-3">
                          <button onClick={() => { if (confirm('¿Eliminar egreso?')) deleteExpense.mutate(e.id); }} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500"><Trash2 className="h-4 w-4" /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <Modal open={showPayment} onClose={() => setShowPayment(false)} title="Registrar Ingreso" size="md">
          <PaymentForm clients={clients} memberships={memberships} onSuccess={() => { setShowPayment(false); qc.invalidateQueries({ queryKey: ['finances-summary'] }); }} onCancel={() => setShowPayment(false)} />
        </Modal>
        <Modal open={showExpense} onClose={() => setShowExpense(false)} title="Registrar Egreso" size="md">
          <ExpenseForm onSuccess={() => { setShowExpense(false); qc.invalidateQueries({ queryKey: ['finances-summary'] }); }} onCancel={() => setShowExpense(false)} />
        </Modal>
